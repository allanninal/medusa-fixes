...
test("exactly at grace boundary is not flagged", () => {
  const exact = order({ created_at: "2026-07-09T23:50:00Z" }); // exactly GRACE_MS before NOW_MS
  const result = findOrdersMissingNotification([exact], [], "order.placed", GRACE_MS, NOW_MS);
  assert.deepEqual(result, []);
});
