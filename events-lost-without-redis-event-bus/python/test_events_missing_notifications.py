...
def test_exactly_at_grace_boundary_is_not_flagged():
    exact = order(created_at="2026-07-09T23:50:00Z")  # exactly GRACE_MS before NOW_MS
    result = find_orders_missing_notification([exact], [], "order.placed", GRACE_MS, NOW_MS)
    assert result == []
